...
const STORAGE_ROOT = process.env.VIDEO_STORAGE_DIR
  ? path.resolve(process.cwd(), process.env.VIDEO_STORAGE_DIR)
  : path.resolve(process.cwd(), 'storage', 'video-generation');
    ...
    for (const asset of assets) {
      ...
      try {
        ...
        if (error.code !== 'ENOENT') {
          console.warn('[VideoStorageService] Failed to remove asset', error);
        }
      }
    }
  }
